...
def main():
    parser = argparse.ArgumentParser(description="IBVAP Phase 3 CLI Tracking Runner")
    ...
    parser.add_argument("--output-path", type=str, default="../data/videos/output_tracking.mp4", help="Path to save annotated video")
    ...
    try:
        while True:
            ...
            # Replace detection list with tracks for annotation
            ...
            if result['frame_number'] % 30 == 0:
                print(f"Frame {result['frame_number']}: {len(active_tracks)} active tracks")
                if active_tracks:
                    print(json.dumps(active_tracks[0], indent=2))
